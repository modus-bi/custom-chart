...
import {getCategoryField, getDatasetId, getIsPinned, getPartsExpression, getPartsParent, getQueryObjectVariables, getTitle, hsQuote} from '../../helpers';
...
export default class CommonDataAdaptor {
  ...
  refresh(data, config, spec, cacheId) {
    ...
    if (data) {
      ...
      this.source[datasetId] = (
        CommonDataAdaptor.seriesLimitToShow(
          CommonDataAdaptor.rowsLimitToShow(
            data, config
          ),
          config
        )
  ...
  remapData(config) {
    ...
    const identityAxe = _.find(config.axes, ['type', 'identity']) || { fields: [] };
    const linkageAxe = _.find(config.axes, ['type', 'linkage']) || { fields: [] };
    const keysAxe = _.find(config.axes, ['type', 'keys']) || { fields: [] };

    this.plotData = [];
    // если данные не пусты
    if (!_.isEmpty(self.aggregated)) {

      const rowCount = self.aggregated[0].length;
      // remap all rows to one Object { name: value, name: value,... }
      for (let i = 0; i < rowCount; i += 1) {
        ...
        _.forEach(self.aggregated, (trace) => {
          const elem = trace[i];

          if (_.isEmpty(elem)) return;

          const valueField = _.find(_.concat(valueAxe.fields, identityAxe.fields, linkageAxe.fields, keysAxe.fields) || [], ['id', elem.id]) || {}; // source valueAxis fied

          // Специальные типы полей
          if (valueField && valueField.valueType && valueField.valueType !== 'value') {
            switch (valueField.valueType) {
              ...
              case 'color' :
                ...
                newRow.color = tinycolor(elem.values).toRgbString();
                break;

              case 'info':
                newRow['__info'].push({
                  title: valueField.doShowTitleForInfo ? (valueField.title || valueField.alias) : '',
                  value: elem.values
                });
                break;

              default :
                // Типы полей "Минимум", "Максимум", "Подзаголовок", "Изображение"...
                newRow[valueField.valueType] = elem.values;
                break;
            }
          }
          // Поля типа "Значение"
            ...
            //if (valueVisible) {
            /** Hide field result if needed **/
            if (!valueField.hideResult) {
              if (config.chartType === 'treeGraph') {
                newRow[elem.fieldAlias] = elem.values;
              }
              if ((keysAxe.fields || []).includes(valueField)) {
                //add keys
                keys.push(String(elem.values));
              }
              else {
                newRow[elem.name] = elem.values;
              }
              if (elem.identity) newRow.identity = elem.identity;
              if (elem.linkage) newRow.linkage = elem.linkage;
            }
            //}
          }
        });
        newRow.thresholds = _.sortBy(_.uniqBy(thresholds, 'value'), 'value');
        newRow.keyString = keys.join('_');
        //newRow.categoryField = _.cloneDeep(categoryField);
        self.plotData[i] = newRow;
      }
    }
