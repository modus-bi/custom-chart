...
export default class CommonDataAdaptor {
  ...
  static replaceVariables(calc, config) {
    let calcNew = structuredClone(calc);
  ...
  static getQueryObjects(config_, exportMode = null, settings = {}) {
    const config = structuredClone(config_);
  ...
  static getCalc(field, config) {
    ...
    else if (field.tree) calc = getFormula(replaceCustomFunctions(structuredClone(field.tree)), { calcMode: true });
  ...
  remapData(config) {
    const categoryAxe = _.find(config.axes, ['type', 'categories']) || {};
    const valueAxe = structuredClone(_.find(config.axes, ['type', 'values']) || {});
    ...
    if (getHasTooltipField(config)) {
      const tooltipField = _.find((categoryAxe || {}).fields, (field) => !!field.tooltipBy);
      if (tooltipField) {
    ...
    const thresholds = [];
    const initialData = [];
    const aggregated = structuredClone(this.aggregated);
      ...
      _.map(aggregated, (rowData, index) => {
        ...
        if (!_.isEmpty(rowData)) {
          ...
          if (!valueField.hideResult) {
              ...
              _.merge(
                ...
                _.mapKeys(data, (v, k) => {
                  ...
                  if (valueField.valueType && valueField.valueType !== 'value') {
                    ...
                    // console.log('k !== thresholds', valueField.valueType, v, valueField);
                    /** Normal fields (do not remap categories and series) **/
                    if (k === 'categories') {
                      remapped.categoryField = structuredClone(categoryField);
                    }
            ...
            // console.log(initialData, newRowData);
