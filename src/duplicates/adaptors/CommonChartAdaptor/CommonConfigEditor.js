...
import { getOrderedFields, shiftOrderIndex, clearNotOrderedFieldsOrderIndex } from '../../helpers';
...
export default class CommonConfigEditor {
  ...
  addField(config, axisName, fieldIndex, fieldItem, type) {
    ...
    if (
      config.chartType === 'stock' &&
      axisType === 'categories' &&
      !['date', 'calculated'].includes(fieldItem.type)) {
      return config;
    }
  ...
  addAllFields(config, datasetId, datasets) {
    ...
    axe.fields = [];
    _.each(dataset.fields, (fieldItem) => {
      // создаем поле на базе драг-итема
      const fieldNew = _.defaults(
        _.merge({}, _.omit(fieldItem, ['axisName', 'fieldIndex', 'itemType']), { id: fieldItem.name + '_' + nanoid() }),
        defaultFieldOptions
  ...
  setAggregationAuto(axes) {
    ...
    if (
      categoriesAxe &&
      categoriesAxe.fields.length > 0 &&
      categoriesAxe.fields[categoriesAxe.selectedFieldIndex]
    ) {
      categoriesFieldName = categoriesAxe.fields[categoriesAxe.selectedFieldIndex].name;
    }

    if (categoriesAxe) {
      _.each(categoriesAxe.fields, field => {
        delete field.agg;
      });
    }
    ...
    if (
      ...
      seriesAxe.fields[seriesAxe.selectedFieldIndex]
    ) {
      seriesFieldName = seriesAxe.fields[seriesAxe.selectedFieldIndex].name;
    }

    if (seriesAxe) {
      _.each(seriesAxe.fields, field => {
        delete field.agg;
      });
    }
