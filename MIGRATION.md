...
на момент копирования, обновляется вручную при обновлении ядра.

Тело снимка держите **дословным**, а сверху допишите шапку с `contractVersion`, `coreVersion`
(оба — из `prebuild/api/version.json`), датой копирования и запретом ручной правки. Тогда
расхождение с ядром ловится обычным `diff`, а всё, кроме шапки, в нём — настоящая находка.
Порядок сверки после обновления ядра — скилл
[`sync-contract`](.claude/skills/sync-contract/SKILL.md).

Затем добавьте в `src/index.ts` проверку набора экспортов на этапе компиляции:
...
2. `defaultConfig.json` → рядом заводится типизированное зеркало (`ChartConfig` в
   `plugin.types.ts`) и `getDefaultConfig.ts`. В шаблоне все три лежат в
   `src/modules/CustomChart/model/` — в мигрируемом плагине их можно оставить там, где они
   были: раскладка папок к переводу типов отношения не имеет (см. ниже).
...
### Раскладка файлов — отдельно и потом

Шаблон раскладывает модуль `CustomChart` по роду файла: `model/` (типы, конфиг, чистые
преобразования данных), `ui/` (компоненты подложки), `hooks/`, `lib/` (чистые функции),
`engine/` (мост к библиотеке отрисовки), а наружу отдаёт себя одним `index.ts`. Правила разбора
границ — скилл [`custom-chart-module`](.claude/skills/custom-chart-module/SKILL.md).

**Переносить эту раскладку во время миграции не нужно.** Перевод типов и перекладывание файлов —
две разные работы, и смешивать их дорого: когда сборка падает, непонятно, виноват типовой
перевод или переехавший импорт. Переводите файлы там, где они лежат; раскладку, если она нужна,
делайте отдельным проходом после зелёного `npm run typecheck && npm test && npm run build` —
тогда каждый переезд это `git mv` плюс правка импортов, без единой правки логики.
...
Каталог `.claude/` едет вместе с шаблоном целиком, поэтому после миграции у плагина оказываются
и остальные скиллы: правила языка (`typescript-coding-standards`), цикл разработки через тесты
(`tdd`), заведение настройки панели (`new-setting`), сверка контракта после обновления ядра
(`sync-contract`) и разборы отдельных модулей. Они написаны про этот шаблон, но говорят о
контракте с ядром, а не о конкретном плагине, — в мигрированном репозитории работают так же.
Перечень с условиями вызова — в [CLAUDE.md](CLAUDE.md), раздел «Работа ИИ-агентов».
